<x-admin>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.result.store') }}" method="POST" id="resultForm">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="study_center" class="form-control" value="{{ old('study_center') }}" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="passing_year" class="form-control" value="{{ old('passing_year') }}" min="1900" max="{{ date('Y') + 1 }}" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>GPA/CGPA *</label>
                            <input type="number" step="0.01" name="gpa_cgpa" class="form-control" value="{{ old('gpa_cgpa') }}" min="0" max="4.00" required>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-check">
                                    <input class="form-check-input semester-checkbox" type="checkbox" 
                                           ...
                                           {{ in_array($semester, old('selected_semesters', [])) ? 'checked' : '' }}>
                ...
                <div id="semesterForms">
                    @if(old('selected_semesters'))
                        @foreach(old('selected_semesters') as $semester)
                                                ...
                                                <tr>
                                                    <td>
                                                        <input type="number" 
                                                               ...
                                                               max="2030"
                                                               placeholder="Year"
                                                               value="{{ old("semester_results.$semester.$i.year") }}">
                                                    </td>
                                                    <td>
                                                        <input type="text" 
                                                               ...
                                                               value="{{ old("semester_results.$semester.$i.course_code") }}">
                                                    </td>
                                                    <td>
                                                        <input type="text" 
                                                               ...
                                                               value="{{ old("semester_results.$semester.$i.course_name") }}">
                                                    </td>
                                                    <td>
                                                        <select name="semester_results[{{ $semester }}][{{ $i }}][grade]" 
                                                            ...
                                                            @foreach(['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-','D', 'F', 'P', 'I'] as $grade)
                                                                <option value="{{ $grade }}" {{ old("semester_results.$semester.$i.grade") == $grade ? 'selected' : '' }}>
                                                                    {{ $grade }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control" required>
                        <option value="on" {{ old('status') == 'on' ? 'selected' : '' }}>Active</option>
                        <option value="off" {{ old('status') == 'off' ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Semester form system initialized');
            ...
            checkboxes.forEach(checkbox => {
                ...
                // Generate forms for pre-checked checkboxes (form validation errors)
                if (checkbox.checked && !document.getElementById('form_' + checkbox.value.replace(/\s+/g, '_'))) {
                    semesterForms.insertAdjacentHTML('beforeend', generateSemesterForm(checkbox.value));
                }
            });
